...
class Solution(object):
    ...
    def minimum_spanning_tree(self, n, connections):
        ...
        total_spanning_cost = 0
        graph = self.build_graph(connections)
        #print(graph)
        edges = [(0, starting_vertex)]
        #print("edges {}".format(edges))
        heapq.heapify(edges)

        while edges:
            cost, to_vtx = heapq.heappop(edges)
            #print("picking cost {}  to {}".format(cost, to_vtx))
            if to_vtx not in visited:
                visited.add(to_vtx)
                # add up the chosen cost
                total_spanning_cost += cost
                for cst, to_vtx in graph.get(to_vtx, []):
                    heapq.heappush(edges, (cst, to_vtx))

        if len(visited) != n:
            return -1

        return total_spanning_cost
    ...
    def build_graph(self, connections):
        ...
        for connection in connections:
            ...
            if source in mp:
                mp[source].append((cost, target))
            else:
                mp[source] = [(cost, target)]

            if target in mp:
                mp[target].append((cost, source))
            else:
                mp[target] = [(cost, target)]
